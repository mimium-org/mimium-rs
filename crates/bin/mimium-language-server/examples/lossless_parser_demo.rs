...
"#;
    ...
    for (i, token) in tokens.iter().take(10).enumerate() {
        println!("  {}: {} at {}:{} = {:?}", 
            i, token.kind, token.start, token.length, token.text(source));
    }
    println!();
    ...
    println!("Leading trivia map entries: {}", preparsed.leading_trivia_map.len());
    println!("Trailing trivia map entries: {}", preparsed.trailing_trivia_map.len());
    println!();
    
    // Step 3: Parse to CST
    println!("Step 3: CST Parsing (Green Tree)");
    let cst = lossless_parser::parse_cst(&tokens, &preparsed);
    println!("Root node kind: {:?}", cst.kind());
    println!("Root node width: {}", cst.width());
    println!();
