...
/// Construct an [`ExecContext`] without plugins for type checking purposes.
/// Language server doesn't need to execute code, so plugins are not required.
fn get_default_context(path: Option<PathBuf>, _with_gui: bool, config: Config) -> ExecContext {
    let plugins: Vec<Box<dyn Plugin>> = vec![];
    ExecContext::new(plugins.into_iter(), path, config)
}
struct MimiumCtx {
