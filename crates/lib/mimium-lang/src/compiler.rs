...
"#
    ...
    fn mutable_upvalue_rust_source() -> &'static str {
        r#"
fn make_counter(){
    let c = 0.0
    |x| {
...
"#;
        ...
        assert!(output.source.contains("pub fn call_dsp_buffer"));
        assert!(output.source.contains("fn counter("));
        assert!(output.source.contains("fn dispatch_counter("));
        assert!(output.source.contains("#[inline(always)]"));
        assert!(!output.source.contains("fn func_"));
    }

    #[test]
    fn emit_rust_compiles_and_runs_buffered_dsp_entrypoint() {
        ...
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let source_path = tmp_dir.join(format!("emit_rust_buffered_{stamp}.rs"));
        let binary_path = tmp_dir.join(format!("emit_rust_buffered_{stamp}"));
    ...
    #[test]
    fn emit_rust_buffered_entrypoint_supports_zero_input_multi_output() {
        let output = test_context()
            .emit_rust(
                r#"
fn phase() {
...
"#,
        ...
        let harness = render_rust_test_main(
            ...
            "let mut program = MimiumProgram::new();",
            None,
            "    let input: [f64; 0] = [];\n    let mut output = [0.0f64; 8];\n    program.call_dsp_buffer(&input, &mut output, 4).unwrap();\n    for sample in output {\n        println!(\"{:.12}\", sample);\n    }\n",
        );
        ...
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let source_path = tmp_dir.join(format!("emit_rust_zero_input_buffered_{stamp}.rs"));
        let binary_path = tmp_dir.join(format!("emit_rust_zero_input_buffered_{stamp}"));
        fs::write(&source_path, format!("{}{harness}", output.source)).unwrap();
        ...
        let actual = String::from_utf8(run.stdout)
            ...
            .map(|line| line.parse::<f64>().unwrap())
            .collect::<Vec<_>>();
        let expected = vec![1.0, 2.0, 2.0, 3.0, 3.0, 4.0, 4.0, 5.0];
        assert_eq!(actual, expected);
    }
    ...
    #[test]
    fn emit_rust_supports_mutable_upvalues() {
        let output = test_context()
            .emit_rust(mutable_upvalue_rust_source())
            .unwrap();
        let harness = render_rust_test_main(
            "",
            "let mut program = MimiumProgram::new();",
        ...
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let source_path = tmp_dir.join(format!("emit_rust_mutable_upvalue_{stamp}.rs"));
        let binary_path = tmp_dir.join(format!("emit_rust_mutable_upvalue_{stamp}"));
        fs::write(&source_path, format!("{}{harness}", output.source)).unwrap();
