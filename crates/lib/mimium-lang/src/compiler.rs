...
pub fn emit_ast(
    ...
    let (ast, _module_env, _visibility_map, errs) = parser::parse_to_expr(src, path.clone());
    if errs.is_empty() {
...
impl Context {
    ...
    pub fn emit_mir(&self, src: &str) -> Result<Mir, Vec<Box<dyn ReportableError>>> {
        ...
        let (ast, _module_env, visibility_map, mut parse_errs) = parser::parse_to_expr(src, path);
        // let ast = parser::add_global_context(ast, self.file_path.unwrap_or_default());
        let mir = mirgen::compile_with_visibility(
            ast,
