...
fn box_gc_test() {
    ...
    assert_eq!(heap_size_1, heap_size_2, 
        "Heap size changed between first and second run: {heap_size_1} vs {heap_size_2}");
    assert_eq!(heap_size_2, heap_size_3, 
        "Heap size changed between second and third run: {} vs {}", 
        heap_size_2, heap_size_3);
    
    // Additionally, the heap should be empty (all objects released)
    assert_eq!(heap_size_3, 0, 
        "Heap is not empty after program completion: {} objects remaining", 
        heap_size_3);
    
    println!("✓ Box GC test passed: heap properly cleaned up after each run");
}
...
fn box_gc_stress_test() {
    ...
    for i in 0..10 {
        ...
        if i > 0 {
            assert_eq!(heap_size, heap_sizes[0], 
                "Heap size changed on iteration {}: expected {}, got {}", 
                i, heap_sizes[0], heap_size);
        }
    }
    
    // All heap sizes should be zero
    assert!(heap_sizes.iter().all(|&size| size == 0), 
        "Some runs left objects in heap: {:?}", heap_sizes);
    
    println!("✓ Box GC stress test passed: no memory leaks over {} iterations", heap_sizes.len());
}
